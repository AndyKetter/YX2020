  ...
  methods: {
    deleted(row) { // 操作删除
      this.$msgBox.confirm("你确定要删除该条数据吗?", "提示", {
        ...
        DELETE(process.env.VUE_APP_DOMAIN_administration + "/ledgerWaterFee", {
          ids: row.id
        }).then(() => {
          this.JS_search();
    ...
    search() {
      ...
      this.loading = true;
      POST(process.env.VUE_APP_DOMAIN_administration + '/ledgerWaterFees', obj).then(res => {
        this.loading = false;
    ...
    confirm(formName) { //添加/修改
      this.$refs[formName].validate(valid => {
        if (valid) {
          const url = process.env.VUE_APP_DOMAIN_administration + '/ledgerWaterFee';
          let obj = {
    ...
    dialogExport() {// 导出
      let url = process.env.VUE_APP_DOMAIN_administration + "/ledgerWaterFeeExport?token="
