  ...
  <div class="dormitoryDed">
    ...
    <div class="bgWhite mgt15 content clearfix">
      ...
      <div class="pull-left">
        <el-form :inline="true" class="mar-no">
          <el-form-item label="姓名">
            <el-input @keyup.enter.native="inputEnter" clearable v-model="name" placeholder="请输入名字" class="width155"></el-input>
          </el-form-item>
          <el-form-item label="员工编号">
            <el-input @keyup.enter.native="inputEnter" clearable v-model="aid" placeholder="请输入编号" class="width155"></el-input>
          </el-form-item>
          <el-form-item label="年月">
            <el-date-picker v-model="batch" type="month" value-format="yyyy-MM" class="width155" placeholder="选择年月"></el-date-picker>
          </el-form-item>
          <el-form-item>
      ...
      <div class="table_wrap">
        <el-table v-loading="loading" :height="tabHeight" :data="dormitoryDedData" border style="width:100%" :header-cell-style="{backgroundColor:'#e5e9f3'}">
          ...
          <el-table-column label="宿舍费扣款" min-width="120">
            <template slot-scope="scope">
              {{scope.row.dormitoryDeductions | toThousands}}
            </template>
          </el-table-column>
    ...
    <el-dialog title="修改扣款项" :before-close="handleModifyClose" :close-on-click-modal="false" :visible.sync="updateCondition" width="768px" class="grayBg">
      ...
      <div class="whiteBox">
        ...
        <el-form :model="updateConditionform" :rules="updateConditionformRule" ref="updateConditionform" label-width="150px">
          <el-row>
            <el-col :md="24">
              <el-form-item label="宿舍扣款：" prop="dormitoryDeductions">
                <el-input-number :controls="false" v-model="updateConditionform.dormitoryDeductions" :min="0" :max="10000000"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :md="24">
...
export default {
  ...
  data() {
    const uploadActionUrl = process.env.VUE_APP_DOMAIN_administration + '/dormitorysImportData' //导入action  线上
    ...
    return {
      ...
      updateCondition: false, //扣款项弹框
      modifyForm: {}, //修改对象
      updateConditionform: {
        //修改条件
        dormitoryDeductions: undefined, //宿舍扣款
        remark: "" //备注
      },
      updateConditionformRule: {
        dormitoryDeductions: [
          { required: true, message: "请填写宿舍扣款", trigger: "blur" }
        ], //宿舍扣款
        remark: [
  ...
  methods: {
    ...
    ignoreSubmit(id, confirmStatus, msg) {//忽略,取消忽略上报
      ...
      this.$msgBox
        ...
        .then(() => {
          PUT(process.env.VUE_APP_DOMAIN_personnel + "/dormitoryIgnore", { ids: [id], confirmStatus, msg })
            .then(res => {
              //单条忽略,取消忽略上报
              this.$message({
                type: "success",
    ...
    search() {
      ...
      POST(process.env.VUE_APP_DOMAIN_administration + "/dormitorys", searchObj)
        .then(res => {
    ...
    unloadErrorLink() {//导入错误模板下载
      ...
      let url = process.env.VUE_APP_DOMAIN_download + '/oss/download?key=' + this.downloadErrorKey + '&token=' + localStorage.getItem("token") + "&name=" + encodeURI("住宿扣款项失败明细") + ".xlsx";
    ...
    resetSubmit() { //重置数据 
      this.$msgBox.confirm(
        ...
        PUT(process.env.VUE_APP_DOMAIN_administration + "/dormitoryReset",
          {
            batch: this.batch,
            confirmStatus: 0
          }).then(res => {
    ...
    JS_download() {//下载模板
      GET(process.env.VUE_APP_DOMAIN_personnel + "/queryBizFilesTemplate", { fileCode: 'ADMIN_DORMITORY_IMPORT_TEMPLATE' })
        .then(res => {
          //获取模板存储返回的ID
    ...
    exportData() {//导出
      window.location.href = process.env.VUE_APP_DOMAIN_administration + '/dormitorysExportData?name=' + this.name + '&batch=' + this.batch + '&aid=' + this.aid + '&token=' + localStorage.getItem('token') //window.location.href方式打开二进制文件下载文件
    },
    updateThis(id) {
      //修改 查看详情
      this.updateCondition = true; //弹出修改框
      GET(process.env.VUE_APP_DOMAIN_administration + "/dormitory", { id: id })
        .then(res => {
          //获取宿舍扣款详细数据
          this.modifyForm = res.data;
          console.log("获取宿舍扣款详细数据", res);
    ...
    JS_update_save(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          PUT(process.env.VUE_APP_DOMAIN_administration + "/dormitory", saveObj)
            .then(res => {
              this.$message({
                type: "success",
                message: "修改数据成功!",
    ...
    deleteThis(id) {//单个删除
      this.$msgBox.confirm('你确定要删除该条数据吗?', '提示', {
        ...
        DELETE(process.env.VUE_APP_DOMAIN_administration + '/dormitorys', { "ids": id }).then(res => { //单条数据删除
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
    ...
    deleteAll() {//按批次删除
      if (this.batch == '') {//如果没选择年月
        ...
        this.$msgBox.confirm('你确定要删除该月所有数据吗?', '提示', {
          ...
          DELETE(process.env.VUE_APP_DOMAIN_administration + '/dormitorysToBatch', { "batch": this.batch }).then(res => { //按批次删除
            this.$message({
              type: 'success',
              message: '批次删除成功!'
            })
    ...
    isLock() {//判断数据是否上锁
      GET(process.env.VUE_APP_DOMAIN_administration + "/dormitoryCheckFinancialDataLock", { batch: this.batch }).then(res => {
    ...
    JS_report_data() {
      //上报数据
      this.$msgBox.confirm(
        '<span>请确认你上传的' + this.batch + '月份</span><span style="color:red;">【住宿扣款】</span><span>无误，确认提交后，将直接影响工资准确发放。</span>',
        "温馨提示",
        {
          dangerouslyUseHTMLString: true,
          confirmButtonText: "确定",
        ...
        GET(process.env.VUE_APP_DOMAIN_administration + "/dormitoryCollectEmployPayroll", { batch: this.batch }).then(res => {
    ...
    JS_cancel_report() {
      ...
      this.$msgBox.confirm(
        ...
        GET(process.env.VUE_APP_DOMAIN_administration + "/dormitoryCancelSubmit", { batch: this.batch }).then(res => {
  ...
  created() {
    ...
    // GET('/api/getDormitoryDedData').then(res=>{//mock宿舍扣款
    //   this.dormitoryDedData = res.dataTable
    // }).catch(error=>{
    //     console.log(error)
    // })
  }
