  ...
  <div class="administrativeDed">
    <div class="guide">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>绩效工资</el-breadcrumb-item>
    ...
    <div class="bgWhite mgt15 content clearfix">
      ...
      <div class="pull-left">
        <el-form :inline="true" class="mar-no">
          <el-form-item label="姓名">
            <el-input clearable @keyup.enter.native="inputEnter" v-model.trim="name" placeholder="请输入名字" class="width155"></el-input>
          </el-form-item>
          <el-form-item label="员工编号">
            <el-input-number @keyup.enter.native="inputEnter" :controls="false" v-model.trim="aid" class="width155" placeholder="请输入编号" :min="0" :max="10000000000"></el-input-number>
          </el-form-item>
      ...
      <div class="table_wrap">
        <el-table v-loading="loading" :height="tabHeight" :data="PerformanceDedData" border style="width:100%" :header-cell-style="{backgroundColor:'#e5e9f3'}">
          ...
          <el-table-column label="绩效考核" min-width="100">
            <template slot-scope="scope">
              {{scope.row.performanceAppraisal | toThousands}}
            </template>
          </el-table-column>
          <el-table-column label="节日福利" min-width="100">
            <template slot-scope="scope">
              {{scope.row.festivalWeal | toThousands}}
            </template>
          </el-table-column>
          <el-table-column label="年终奖励" min-width="100">
            <template slot-scope="scope">
              {{scope.row.annualHonus | toThousands}}
            </template>
          </el-table-column>
    ...
    <!-- 修改扣款项弹框 -->
    <el-dialog title="修改" :close-on-click-modal="false" :before-close="handleModifyClose" :visible.sync="updateCondition" width="768px" class="grayBg">
      <div class="whiteBox">
      ...
      <div class="whiteBox">
        ...
        <el-form :model="updateConditionform" :rules="updateConditionformRule" ref="updateConditionform" label-width="150px">
          <el-row>
            <el-col :md="12">
              <el-form-item label="绩效考核：" prop="performanceAppraisal">
                <el-input-number :controls="false" v-model="updateConditionform.performanceAppraisal" style="width:200px" :min="0" :max="10000000"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :md="12">
              <el-form-item label="节日福利：" prop="festivalWeal">
                <el-input-number :controls="false" v-model="updateConditionform.festivalWeal" style="width:200px" :min="0" :max="10000000"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :md="12">
              <el-form-item label="年终奖励：" prop="annualHonus">
                <el-input-number :controls="false" v-model="updateConditionform.annualHonus" style="width:200px" :min="0" :max="10000000"></el-input-number>
              </el-form-item>
            </el-col>
    ...
    <!-- 扣款项导入弹框 -->
    <el-dialog :before-close="fileClose" title="绩效工资导入" :close-on-click-modal="false" :show-close="false" :visible.sync="uploadModal" width="520px">
      <p class="clearfix">
        <el-upload ref="upload" class="pull-left" :action="uploadActionUrl" :headers="headers" :before-remove="beforeRemove" :before-upload="beforeUpload" :on-success="handleSuccess" :file-list="fileList">
...
export default {
  ...
  data() {
    const uploadActionUrl = process.env.VUE_APP_DOMAIN_personnel + '/performanceSalarysImportData' //导入action  线上
    ...
    return {
      ...
      updateConditionform: {//修改条件
        performanceAppraisal: undefined,//绩效考核
        festivalWeal: undefined,//节日福利
        annualHonus: undefined,//年终奖励
        remark: ''//备注
      },
      updateConditionformRule: {
        performanceAppraisal: [{ required: true, message: "请填写绩效考核", trigger: "blur" }],
        festivalWeal: [{ required: true, message: "请填写节日福利", trigger: "blur" }],
        annualHonus: [{ required: true, message: "请填写年终奖励", trigger: "blur" }],
        remark: [
  ...
  methods: {
    ...
    ignoreSubmit(id, confirmStatus, msg) {//忽略,取消忽略上报
      ...
      this.$msgBox
        ...
        .then(() => {
          PUT(process.env.VUE_APP_DOMAIN_personnel + "/performanceSalarysIgnore", { ids: [id], confirmStatus, msg })
            .then(res => {
              //单条忽略,取消忽略上报
              this.$message({
                type: "success",
    ...
    search() {
      ...
      POST(process.env.VUE_APP_DOMAIN_personnel + '/performanceSalarys', searchObj).then(res => {
        this.loading = false //关闭动画
        this.PerformanceDedData = res.data.records //set表格的值
    ...
    unloadErrorLink() {//导入错误模板下载
      ...
      let url = process.env.VUE_APP_DOMAIN_download + '/oss/download?key=' + this.downloadErrorKey + '&token=' + localStorage.getItem("token") + "&name=" + encodeURI("绩效工资失败明细") + ".xlsx";
    ...
    JS_download() {//下载模板
      GET(process.env.VUE_APP_DOMAIN_personnel + "/queryBizFilesTemplate", { fileCode: 'PERSONNEL_PERFORMANCESALARY_IMPORT_TEMPLATE' })
        .then(res => {
          //获取模板存储返回的ID
    ...
    resetSubmit() { //重置数据 
      this.$msgBox.confirm(
        ...
        PUT(process.env.VUE_APP_DOMAIN_personnel + "/performanceSalarysReset",
          {
            batch: this.batch,
            confirmStatus: 0
          }).then(res => {
    ...
    exportData() {//导出
      window.location.href = process.env.VUE_APP_DOMAIN_personnel + '/performanceSalarysExportData?name=' + this.name + '&batch=' + this.batch + '&aid=' + (this.aid || '') + '&token=' + localStorage.getItem('token') //window.location.href方式打开二进制文件下载文件
    },
    updateThis(id) {//修改 查看详情
      this.updateCondition = true //弹出修改框
      GET(process.env.VUE_APP_DOMAIN_personnel + '/performanceSalary', { "id": id }).then(res => {//获取员工扣款详细信息
    ...
    JS_update_save(formName) {//修改保存
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          PUT(process.env.VUE_APP_DOMAIN_personnel + '/performanceSalary', saveObj).then(res => {
            this.$message({
              type: 'success',
              message: '修改数据成功!',
              showClose: true
    ...
    deleteThis(id) {//单个删除
      this.$msgBox.confirm('你确定要删除该条数据吗?', '提示', {
        ...
        DELETE(process.env.VUE_APP_DOMAIN_personnel + '/performanceSalarys', { "ids": id }).then(res => { //单条数据删除
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
    ...
    deleteAll() {//按批次删除
      if (this.batch == '') {//如果没选择年月
        ...
        this.$msgBox.confirm('你确定要删除该月所有条数据吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          DELETE(process.env.VUE_APP_DOMAIN_personnel + '/performanceSalarysToBatch', { "batch": this.batch }).then(res => { //按批次删除
            this.$message({
              type: 'success',
              message: '批次删除成功!'
            })
    ...
    isLock() {//判断数据是否上锁
      GET(process.env.VUE_APP_DOMAIN_personnel + "/performanceSalarysCheckFinancialDataLock", { batch: this.batch }).then(res => {
    ...
    JS_report_data() {
      //上报数据
      this.$msgBox.confirm(
        '<span>请确认你上传的' + this.batch + '月份</span><span style="color:red;">【绩效工资】</span><span>无误，确认提交后，将直接影响工资准确发放。</span>',
        "温馨提示",
        {
          dangerouslyUseHTMLString: true,
          confirmButtonText: "确定",
        ...
        GET(process.env.VUE_APP_DOMAIN_personnel + "/performanceSalarysCollectEmployPayroll", { batch: this.batch }).then(res => {
    ...
    JS_cancel_report() {
      ...
      this.$msgBox.confirm(
        ...
        GET(process.env.VUE_APP_DOMAIN_personnel + "/performanceSalarysCancelSubmit", { batch: this.batch }).then(res => {
