<template>
  <div class="inbox">
        <div class="clearfix">
          <div class="pull-left inboxTitle">已发送</div>
        ...
        <div class="fuctAreas">
          ...
          <el-button icon="el-icon-refresh-left" @click="backMailClick">撤销邮件</el-button>
    ...
    <ul class="mailList mgt10" :style="{height:mailListHeight}">
      <li :class="{hadRead:item.isRead}" v-for="(item,index) in emailDataList" :key="item.id">
         <el-row :gutter="0">
          ...
          <el-col :span="10" class="flex pointer" @click.native="toSentDts(index,'sent')">
  ...
  methods:{
    ...
    backMailClick(){//撤销邮件
      ...
      if(itemCheckedLength == 0){ //没有勾选
        this.$msgBox.alert('请选择要撤销的邮件！','温馨提示',{
        ...
        this.$msgBox.confirm('确定要撤销勾选邮件？','温馨提示',{
          confirmButtonText:'确定',
          cancelButtonText:'取消',
          type:'warning'
        }).then(()=>{
