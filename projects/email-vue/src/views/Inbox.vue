<template>
  <div class="inbox">
        <div class="clearfix">
          <div class="pull-left inboxTitle">收件箱</div>
          <el-input placeholder="搜索" clearable v-model="search" class="searchInpt pull-right">
        ...
        <div class="fuctAreas">
          <el-button-group class="mar6" style="position:relative">
            ...
            <div class="togglePanel" v-show="play">
              ...
              <ul style="border-top:1px solid #e5e5e5;padding-top:13px">
                <li @click="readedClick">已读</li>
                <li @click="unreadedClick">未读</li>
              </ul>         
          </div>
          </el-button-group>
          <el-button icon="el-icon-refresh" @click="refreshClick"></el-button>
          <el-button style="margin-left:6px;padding:8px 15px" @click="signReaded"><img src="../images/readed.png" style="width:13px;vertical-align:bottom">&nbsp;标记已读</el-button>
          <el-button icon="el-icon-message" style="margin:0 6px" @click="signUnReaded">标记未读</el-button>
          <el-button-group class="mar6">
            <el-button icon="el-icon-star-off">添加标签邮件</el-button>
            <el-button icon="el-icon-caret-bottom" @click="toggle2"></el-button>
          </el-button-group>
          <div class="togglePanel2" v-show="play2">
              <ul>
                <li>重要邮件1</li>
                <li>重要邮件2</li>
              </ul>
              <ul style="border-top:1px solid #e5e5e5;padding-top:13px">
                <li @click="addTags">新建标签</li>
              </ul>         
          </div>
      ...
      <li :class="{hadRead:item.isRead}" v-for="(item,index) in emailDataList" :key="item.id">
         <el-row :gutter="0">
          <el-col :span="8" class="flex">
            ...
            <span :class="{boldText:!item.isRead}" class="text-overflow widthlimit1 pointer" @click="searchAddress(item.emailAddresser,'inbox')" v-html="item.emailAddresser"></span>
          </el-col>
          <el-col :span="10" class="flex pointer" @click.native="toInboxDts(index,'inbox')">
    ...
    <!-- 标记已读弹框 -->
    <el-dialog title="提示" :visible.sync="readedDialog" width="400px">
      <div style="text-align:center;">
        <i class="el-icon-warning"></i>&nbsp;请选择要标记为已读的邮件！
      </div>
      <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="readedDialog=false">确定</el-button>
        </span>
    </el-dialog>
   <!-- 标记未读弹框 -->
   <el-dialog title="提示" :visible.sync="unreadedDialog" width="400px">
      <div style="text-align:center;">
        <i class="el-icon-warning"></i>&nbsp;请选择要标记为未读的邮件！
      </div>
      <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="unreadedDialog=false">确定</el-button>
        </span>
    </el-dialog>
     <!-- 新建标签弹框 -->
...
export default {
    ...
    return {
      ...
      emailDataList:[
        ...
        },
        {
          ...
          isHaveNotice:false, //是否有通知
          isHaveAttach:true, //是否有附件
          emailAddresser:'钱骏[rsb@hnyongxiong.cn]', //发件人
          emailTitle:'关于五月试用期员工购买社保的通知', //邮件标题
          emailTime:'2019-05-10 10:17:02' //邮件时间
        },
        {
  ...
  methods:{
    ...
    readedClick(){//点击未读下拉li
      this.emailDataList = this.emailDataList.filter((item)=>{
        item.checked = false
    ...
    signReaded(){//标记已读
      ...
      this.emailDataList.forEach((item)=>{
        ...
        item.checked = false //去掉勾选
      })
      this.isIndeterminate = false
      this.checkAll = false //去掉全选的勾选
      if(itemCheckedLength == 0){ //没有勾选
        this.readedDialog=true
      }
    },
    signUnReaded(){//标记未读
      let itemCheckedLength = 0 //选中多少邮箱列表数据
