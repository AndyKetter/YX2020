<template>
  <div class="inbox">
        <div class="clearfix">
          <div class="pull-left inboxTitle">回收站</div>
        ...
        <div class="fuctAreas">
          ...
          <el-button icon="el-icon-close" @click="deleteMailClick">彻底删除</el-button>
          <el-button icon="el-icon-refresh-left" @click="backMailClick">还原邮件</el-button>
    ...
    <ul class="mailList mgt10" :style="{height:mailListHeight}">
      <li :class="{hadRead:item.isRead}" v-for="(item,index) in emailDataList" :key="item.id">
         <el-row :gutter="0">
          ...
          <el-col :span="10" class="flex pointer" @click.native="toRecycleDts(index,'recycle')">
  ...
  methods:{
    ...
    backMailClick(){//还原邮件
      ...
      if(itemCheckedLength == 0){ //没有勾选
        this.$msgBox.alert('请选择要还原的邮件！','温馨提示',{
        ...
        this.$msgBox.confirm('确定要还原勾选邮件？','温馨提示',{
          confirmButtonText:'确定',
          cancelButtonText:'取消',
          type:'warning'
        }).then(()=>{
    ...
    deleteMailClick(){//删除邮件
      ...
      if(itemCheckedLength == 0){ //没有勾选
        this.$msgBox.alert('请选择要彻底删除的邮件！','温馨提示',{
          confirmButtonText:'确定',
          callback:()=>{
        ...
        this.$msgBox.confirm('确定要彻底删除勾选邮件？','温馨提示',{
          confirmButtonText:'确定',
          cancelButtonText:'取消',
          type:'warning'
        }).then(()=>{
