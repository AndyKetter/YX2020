    ...
    mounted(){
        ...
        this.editor.customConfig.uploadImgHooks = {
            ...
            customInsert:function(insertImg,result,editor){
                ...
                //举例：假如上传图片成功后，服务端返回的是{url:'...'}这种格式，即可这样插入图片：
                let url = Object.values(result.data) // result.data就是服务端返回的图片名字和链接
                JSON.stringify(url) //在这里转成json格式
                insertImg(url) 
                //result必须是一个json格式字符串！！！否则报错
            }
        }
        this.editor.create() //创建富文本实例
        if(!this.content){
            this.editor.txt.html('<p>请编辑内容</p>')
        }
    }
}
</script>
